...
/*   Updated: 2024/02/23 12:52:18 by wvan-der         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_imgs(t_cube *cube)
{
	if (cube->side == 0)
	{
		if (cube->map_x > cube->pos_x)
			cube->img_data = mlx_get_data_addr(cube->so_img, &cube->bits_per_pixel, &cube->size_line, &cube->endian);
		else
			cube->img_data = mlx_get_data_addr(cube->no_img, &cube->bits_per_pixel, &cube->size_line, &cube->endian);
	}
	else
	{
		if (cube->map_y > cube->pos_y)
			cube->img_data = mlx_get_data_addr(cube->ea_img, &cube->bits_per_pixel, &cube->size_line, &cube->endian);
		else
			cube->img_data = mlx_get_data_addr(cube->we_img, &cube->bits_per_pixel, &cube->size_line, &cube->endian);
	}
	if (!cube->img_data)
		render_exit(cube);
}
...
void	calc_tex_height_step_tex_pos_and_draw(t_cube *cube, int x)
{
	...
	cube->step = 1.0 * texture_height / cube->line_height;
	texture_pos = (cube->draw_start - HEIGHT / 2 + cube->line_height / 2) * cube->step;
